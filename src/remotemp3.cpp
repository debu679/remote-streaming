...
int remotehost_Mp3_pipeline(int argc, char *argv[])
{
    ...
    if (!remote_host.pipeline || !remote_host.udp_source || !remote_host.watchdog || !remote_host.rtp_depay || !remote_host.parser || !remote_host.audio_decoder || !remote_host.queue || !remote_host.audio_convert || !remote_host.audio_sink)
    {
    ...
    /* Link the elements */
    if (gst_element_link_many(remote_host.udp_source, remote_host.watchdog, remote_host.rtp_depay,
                              remote_host.parser, remote_host.audio_decoder, remote_host.queue,
                              remote_host.audio_convert, remote_host.audio_sink, NULL) != TRUE)
    {
    ...
    g_signal_connect(bus, "message", G_CALLBACK(callback_message),
                     &data);
