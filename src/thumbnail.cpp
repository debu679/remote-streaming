...
int thumbnail_pipeline(int argc, char *argv[]){
    GstBus *bus;
    ...
    /* Create the elements */
    remote_host.pipeline = gst_pipeline_new ("mypipeline");
    ...
    gst_bin_add_many (GST_BIN (remote_host.pipeline), remote_host.udpsrc, remote_host.watchdog, remote_host.caps_filter, remote_host.rtp_depay, remote_host.decoder, remote_host.videoconvert, remote_host.sink, NULL);

    /* Link the elements together */
    if (!gst_element_link_many (remote_host.udpsrc, remote_host.watchdog, remote_host.caps_filter, remote_host.rtp_depay, remote_host.decoder, remote_host.videoconvert, remote_host.sink, NULL)) {
